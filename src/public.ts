...
async function getShare(env: Env, token: string): Promise<ShareWithFile | null> {
  return await env.db.prepare(
    `SELECT s.id, s.file_id, s.created_at, s.expires_at, s.max_downloads, s.download_count, s.revoked, s.password_hash,
            s.download_name, f.key, f.name, f.size, f.mime, f.meta
...
async function getDirectLink(env: Env, token: string): Promise<DirectLinkWithFile | null> {
  return await env.db.prepare(
    `SELECT dl.id, dl.file_id, dl.created_at, dl.expires_at, dl.max_downloads, dl.download_count, dl.revoked,
            dl.download_name, f.key, f.name, f.size, f.mime, f.meta
...
/**
 * 分块下载：/s/:token/chunk/:index
 * 从文件的 meta 字段里获取 uploadId，然后从 R2 读取对应的分块
 */
export async function handleChunkDownload(
  req: Request,
  env: Env,
  token: string,
  chunkIndex: number
  ...
  const row = await env.db.prepare(
    `SELECT s.id, s.file_id, s.revoked, s.expires_at, s.max_downloads, s.download_count,
            f.key, f.name, f.size, f.mime, f.meta
     FROM shares s JOIN files f ON f.id = s.file_id
     WHERE s.id = ?1`
  )
    .bind(token)
  ...
  if (!row) return errorPage(req, 404, { zh: "分享不存在", en: "Share Not Found" }, {});
  if (row.revoked) return errorPage(req, 410, { zh: "分享已被撤销", en: "Share Revoked" }, {});
  if (row.expires_at && row.expires_at < Date.now()) {
    return errorPage(req, 410, { zh: "分享已过期", en: "Share Expired" }, {});
  }

  // 解析 meta 字段
  if (!row.meta) return errorPage(req, 400, { zh: "文件不是分卷格式", en: "Not chunked" }, {});
  
  let meta: any;
  try {
    meta = JSON.parse(row.meta);
  } catch {
    return errorPage(req, 400, { zh: "元数据格式错误", en: "Bad meta" }, {});
  }

  if (!meta.isChunked || !meta.uploadId) {
    return errorPage(req, 400, { zh: "文件不是分卷格式", en: "Not chunked" }, {});
  }

  if (chunkIndex < 0 || chunkIndex >= meta.totalChunks) {
    return errorPage(req, 404, { zh: "分块不存在", en: "Chunk not found" }, {});
  }
