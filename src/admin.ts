...
export async function handleAdminApi(
  ...
  // ── 分块上传：上传单个分块 ──────────────────────────────
  if (path === "/api/admin/upload-chunk" && method === "POST") {
    ...
    if (!uploadId || !fileName) {
      return json({ error: msg(req, "缺少上传参数", "Missing upload parameters") }, 400);
    }
    
    if (!req.body) {
      return json({ error: msg(req, "请求体为空", "Empty request body") }, 400);
    }
    ...
    try {
      await st.put(chunkKey, req.body, { contentType: mime });
    } catch (err: any) {
      return json({ error: msg(req, "分块上传失败", "Chunk upload failed"), detail: String(err?.message || err) }, 500);
    }
  ...
  if (path === "/api/admin/upload-complete" && method === "POST") {
    ...
    const meta = JSON.stringify({
      ...
      chunkSize: Math.ceil(body.totalSize / body.totalChunks),
    });
    
    try {
      await env.db.prepare(
        "INSERT INTO files(id, key, name, size, mime, uploaded_at, meta) VALUES(?1, ?2, ?3, ?4, ?5, ?6, ?7)"
      )
        .bind(id, key, body.fileName, body.totalSize, mime, Date.now(), meta)
        .run();
    } catch (dbErr) {
      console.error("upload-complete: D1 insert failed:", dbErr);
      return json({ error: msg(req, "数据库写入失败，请重试", "Database write failed. Please retry.") }, 500);
    }
    
    return json({ ok: true, id, name: body.fileName, size: body.totalSize, isChunked: true }, 201);
  }
  // ── 删除文件（连带存储对象、分享、日志） ──────────
